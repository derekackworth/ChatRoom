...
class App extends React.PureComponent {
  ...
  handleRoomChangeChange = (e) => {
    if (e.target.value.toUpperCase() === this.state.roomName) {
      this.setState({
        ...
        roomNameChangeMessage: "You are already in this room",
      });
  ...
  handleLoginKeyPress = (e) => {
    if (
      e.charCode === 13 &&
      this.state.chatNameMessage === "" &&
      this.state.roomNameMessage === ""
    ) {
      this.handleLogin();
    }
  };
  ...
  handleChangeRoom = () => {
    ...
    this.state.socket.emit("changeroom", {
      roomNameChange: this.state.roomNameChange.toUpperCase(),
    });
    this.setState({
  ...
  handleLogout = () => {
    ...
    const socket = io.connect(process.env.REACT_APP_HOST, { forceNew: true });
    this.setState({ socket: socket });
    socket.on("rooms", this.onRooms);
    socket.on("namechange", this.onNameChange);
    socket.on("welcome", this.onWelcome);
    ...
    return (
      <MuiThemeProvider theme={theme}>
        <React.Fragment>
          ...
          >
            ...
            <DialogContent>
              <div style={{ textAlign: "center" }}>
                ...
                />
                <Typography style={{ marginTop: "15px" }} color="primary">
                  Existing rooms:
                </Typography>
                <RadioGroup
          ...
          <Dialog
            open={isWhosOnlineDialogOpen}
            onClose={this.handleCloseWhosOnlineDialog}
          >
            <DialogTitle style={{ textAlign: "center" }}>
              Who"s Online?
            </DialogTitle>
